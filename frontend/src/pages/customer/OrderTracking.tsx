...
export function OrderTracking() {
  ...
  if (!order) return <div className="p-8 text-center text-neutral-500">Loading order…</div>;
        ...
        <div className="mt-4 flex items-center">
          {ORDER_STATUS_FLOW.map((step, idx) => (
            <div key={step} className="flex-1 flex items-center">
              <div
                className={`w-3 h-3 rounded-full ${idx <= activeIndex ? "bg-brand-500" : "bg-neutral-200"}`}
              />
              {idx < ORDER_STATUS_FLOW.length - 1 && (
                <div className={`flex-1 h-0.5 ${idx < activeIndex ? "bg-brand-500" : "bg-neutral-200"}`} />
              )}
            </div>
          ))}
        </div>
